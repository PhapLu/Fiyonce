...
export default function DeleteBadges({
    news,
    setShowDeleteBadges,
    setOverlayVisible,
    deleteBadgesMutation,
}) {
    const [inputs, setInputs] = useState(news);
    const [errors, setErrors] = useState({});
    const [isSubmitDeleteBadgesLoading, setIsSubmitDeleteBadgesLoading] = useState(false);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setIsSubmitDeleteBadgesLoading(true);

        try {
            await deleteBadgesMutation.mutateAsync(news._id);
        } catch (error) {
            ...
            setIsSubmitDeleteBadgesLoading(false);
        }
    };
    ...
    return (
        <div className="delete-commission-service modal-form type-3" ref={deleteCommissionRef} onClick={(e) => { e.stopPropagation(); }}>
            ...
            <div className="form-field">
                <button type="submit" className="form-field__input btn btn-2 btn-md" disabled={isSubmitDeleteBadgesLoading} onClick={handleSubmit}>
                    {isSubmitDeleteBadgesLoading ? 'Đang xóa...' : 'Xác nhận'}
                </button>
            </div>
        </div>
    );
