                        ...
                        <div className="profile-commission-service__category-item">
                            {
                                isProfileOwner &&
                                <>
                                    <button onClick={() => { setUpdateCommissionServiceCategory(category); setShowUpdateCommissionServiceCategoryForm(true); setOverlayVisible(true); }} className="btn btn-7 icon-only profile-commission-service__category-item__update-btn">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
                                            <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                        </svg>
                                    </button>
                                    <button onClick={() => { setDeleteCommissionServiceCategory(category); setShowDeleteCommissionServiceCategoryForm(true); setOverlayVisible(true); }} className="btn btn-7 icon-only profile-commission-service__category-item__delete-btn">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
                            ...
                            <div className="profile-commission-service__category-item__service-container">
                                {category.commissionServices?.map((service, serviceIndex) => (
                                    <Link to={`${location.pathname + "/" + service?._id}`} key={serviceIndex} className="profile-commission-service__category-item__service-item">
                                        ...
                                        {/* {!isProfileOwner &&
                                            <button className="btn btn-7 icon-only bookmark-service-btn">
                                        ...
                                        <div className="profile-commission-service__category-item__service-item--right">
                                            ...
                                            {isProfileOwner ? (
                                                <>
                                                    ...
                                                    <button className="btn btn-4 btn-md" onClick={() => { setDeleteCommissionService(service); setShowDeleteCommissionServiceForm(true); setOverlayVisible(true) }}>Xóa</button>
                                                </>
                                            ) : (
                                                <div className="flex-align-center">
                                                    <button className="btn btn-2 btn-md order-service-now-btn" onClick={() => { setRenderCommissionServiceId(service._id); setShowRenderCommissionService(true); setOverlayVisible(true) }}>Đặt ngay</button>
                                                    <button className="btn btn-4 btn-md">Liên hệ</button>
                                                </div>
                                            )}
                                        </div>
                                    </Link>
