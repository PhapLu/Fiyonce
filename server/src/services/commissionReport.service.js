...
class CommissionReportService{
    ...
    static readCommissionReports = async (userId) => {
        ...
        const user = await User.findById(userId)
        if(!user) throw new NotFoundError('User not found')
        if(user.role !== 'admin') throw new AuthFailureError('User not authorized')
    ...
    static updateCommissionReport = async(userId, commissionReportId, req) => {
        ...
        if(!user) throw new NotFoundError('User not found')
        if(!commissionReport) throw new NotFoundError('Commission Report not found')
        if(commissionReport.clientId.toString() !== userId) throw new AuthFailureError('You can only update your commission report')
        ...
        try {
            ...
            if(req.files && req.files.files && req.files.files.length > 0){
                ...
                const uploadResults = await Promise.all(uploadPromises)
                const evidences = uploadResults.map(result => result.secure_url)
