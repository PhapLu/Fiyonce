        ...
        try {
            ...
            const scoredPosts = await Promise.all(
                posts.map(async (post) => {
                    ...
                    return { ...post.toObject(), score, scaledPostAge, engagementRate }
                })
            )
        ...
        try {
                ...
                CommissionService.aggregate([
                    ...
                    },
                    {
                        $project: {
                            ...
                            followersCount: { $ifNull: [{ $size: '$talent.followers' }, 0] },
            ...
            const scoredServices = await Promise.all(
                services.map(async (service) => {
                    ...
                    // Compute scaled values
                    const orderCountScaled = (service.orderCount - minOrderCount) / (maxOrderCount - minOrderCount)
                    const serviceViewsScaled = (service.views - minServiceViews) / (maxServiceViews - minServiceViews)
                    const followersScaled = (talent.followers.length - minFollowersCount) / (maxFollowersCount - minFollowersCount)
                    const artistViewsScaled = (talent.views - minArtistViews) / (maxArtistViews - minArtistViews)
    
                    // Compute age in hours for scaling
                    const ageInHours = (new Date() - new Date(service.createdAt)) / (60 * 60 * 1000)
                    const ageScaled = (ageInHours - minAgeInHours) / (maxAgeInHours - minAgeInHours)
        ...
        try {
            ...
            const scoredCommissionServices = await Promise.all(
                commissionServices.map(async (service) => {
                    const talent = await User.findById(service.talentId) || { followers: [], views: 0 }
